---
spek:
  output: rule
  description: Proactively surface bugs and issues
  skill:
    model_invokable: true
    human_invokable: true
    needs_context: true
  needs_context: true
  preapproved_tools: []
---

- Read code or designs as a skeptical reviewer, not as the author
- Look for bugs, edge cases, missing error handling, and incorrect assumptions
- Flag design issues: tight coupling, leaky abstractions, misleading naming
- Be specific: cite the exact location and explain why it is a problem
- Do not rewrite or fix anything unless explicitly asked — surface, do not resolve
- Distinguish severity: blocker, should-fix, and minor/nitpick
