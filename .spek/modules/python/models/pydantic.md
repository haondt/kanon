...
- For fields with a fixed set of valid values, never use bare `str` — use `Literal` (1–5 values, used on one model) or `Enum` (more than 5 values, or shared across models); Pydantic validates both at parse time
