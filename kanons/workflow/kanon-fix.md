...
1. Run `kanon session review status --json` to get all review passes. Identify the most recent pass key (e.g. `p2`). Run `kanon session review status --pass <key> --json` to get its findings. If no review passes exist, say so and stop.
...
3. For each finding, in order:
   ...
   d. Record the fix: `kanon session review set-fix-note <pass-key> <finding-key> "<what was done>"`
   e. Close the finding: `kanon session review close-finding <pass-key> <finding-key>`

4. After all fixes are implemented, summarize what was done: one line per finding, referencing its key (e.g. `f1`).

5. Prompt the user to run `/kanon-review` again to validate the fixes.
