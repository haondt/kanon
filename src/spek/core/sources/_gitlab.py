...
class GitLabSource(ParsedSource):
    ...
    @classmethod
    def parse(cls, resolver: SourceResolver, path: str) -> GitLabSource:
        rest, _, ref = path.partition("@")
        ref = ref or None
        parts = rest.split("/")
        if len(parts) < 2 or not all(parts):
            raise ValueError(
                f"Invalid GitLab source path: {path!r}. Expected gl::group/.../repo[@ref]"
            )
    ...
    def serialize(self) -> str:
        ...
        return SourcedResource(GITLAB_SOURCE_TYPE_ID, path).as_string
