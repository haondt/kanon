...
from kanon.core.config import SourceReference, KanonConfig, KanonEnv
from kanon.core.settings import GlobalSettings
from kanon.core.sources import AliasRef, LocalSource, SourceResolver, hydrate_source_reference
from kanon.core.sources._base import PullResult

@click.group()
def source() -> None:
    """Manage named source directories for kanons."""

@source.command("add")
@click.argument("source_name")
@click.argument("path")
@click.option("--global", "-g", "global_scope", is_flag=True, help="Write to ~/.kanon/settings.yaml instead of kanon.yaml.")
...
def source_add(source_name: str, path: str, global_scope: bool, force: bool) -> None:
    ...
    if global_scope:
        ...
        settings.save()
        click.echo(f"Added source '{source_name}' to ~/.kanon/settings.yaml.")
    else:
        ...
        config.save()
        click.echo(f"Added source '{source_name}' to kanon.yaml.")
...
def source_remove(source_name: str, global_scope: bool) -> None:
    ...
    if global_scope:
        ...
        if source_name in settings.sources:
            ...
            settings.save()
        click.echo(f"Removed source '{source_name}' from {KanonEnv.instance().settings_path}.")
    else:
        config = load_config_or_exit()
        if source_name in config.sources:
            del config.sources[source_name]
            config.save()
        click.echo(f"Removed source '{source_name}' from kanon.yaml.")
