...
@click.group()
def ref():
    """Search and read entries from the kanon reference library."""
...
def ref_search(terms: tuple[str, ...], as_json: bool, match_all: bool, limit: int) -> None:
    ...
    if as_json:
        click.echo(json.dumps([
            {
                "name": s,
                "description": r.frontmatter.kanon.description,
                "keywords": r.frontmatter.kanon.keywords
            }
    ...
    for r in results:
        kw = ", ".join(r[2].frontmatter.kanon.keywords)
        desc = r[2].frontmatter.kanon.description or ""
...
def ref_read(name: str, as_json: bool) -> None:
    ...
    if as_json:
        json_data = {
            "name": res.as_string,
            "description": reference.frontmatter.kanon.description,
            "keywords": reference.frontmatter.kanon.keywords,
